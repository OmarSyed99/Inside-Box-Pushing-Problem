...
class InsideBoxPushingEnv(gym.Env):
    """
    Multi‑agent “inside‑box pushing” environment.

    Reward shaping
    --------------
        +100  when the box reaches the target square
        +50   the step the faulty agent is successfully eliminated
        +10 * Δdistance  improvement toward the target each step
        −0.1  per step (small living penalty)
        −25   extra penalty at episode end if the faulty agent is still alive
    """
    ...
    def step(self, actions):
        ...
        if self.enable_voting:
            votes = sum(1 for i, a in enumerate(actions) if a == 5 and self.agents_alive[i] == 1)
            if (
        ...
        for i, a in enumerate(actions):
            ...
            if a == 1:
                ...
                force[0] += self.push_strength
            if i == self.faulty_agent:  # faulty never pushes
                force[:] = 0
            net_force += force
        ...
        # dense shaping for distance reduction
        ...
        if done and self.agents_alive[self.faulty_agent] == 1:
            reward -= 25.0  # discourage finishing with faulty alive
